...
class ReplayBuffer(object):
	...
	def __init__(self, buffer_size, state_space, n_action, n_ant):
		...
		self.pointer = 0
		self.len = 0
		self.states = np.random.rand(self.buffer_size,state_space).astype(np.int32)
		self.actions = np.random.rand(self.n_ant,self.buffer_size,1).astype(np.int32)
		self.rewards = np.random.rand(self.buffer_size,1)
		self.next_states = np.random.rand(self.buffer_size,state_space).astype(np.int32)
